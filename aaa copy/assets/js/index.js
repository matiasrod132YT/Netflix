...
const homePageSections = [
  ...
  {
    title: "Películas de tendencia semanal",
...
fetchDataFromServer(
  ...
  function ({ genres }) {
    ...
    fetchDataFromServer(
      `https://api.themoviedb.org/3/movie/popular?api_key=${api_key}&page=1&language=es-ES`,
      heroBanner
    );
  }
);
...
const heroBanner = function ({ results: movieList }) {
  ...
  banner.ariaLabel = "Peliculas Populares";
    ...
    sliderItem.innerHTML = `
      ...
      index === 0 ? "eager" : "lazy"
        ...
        <div class="banner-content">
          ...
          <div class="meta-list">
            <div class="meta-item">${
              release_date?.split("-")[0] ?? "No publicado"
            }</div>
  ...
  for (const { title, path } of homePageSections) {
    fetchDataFromServer(
      `https://api.themoviedb.org/3${path}?api_key=${api_key}&page=1&language=es-ES`,
      createMovieList,
      title
    );
  }
...
const createMovieList = function ({ results: movieList }, title) {
  ...
  // Crear un contenedor de promesas para las tarjetas de película
  const cardPromises = movieList.map(async (movie) => {
    const movieCard = await createMovieCard(movie); // Pasar el objeto de película
    return movieCard;
  });
